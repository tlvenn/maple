...
// migration:
//   https://github.com/alchemy-run/alchemy-effect/blob/main/packages/alchemy/src/Cloudflare/Workers/DurableObjectStorage.ts
//
// Effect-native wrappers around `cf.DurableObjectStorage`, `cf.SqlStorage`,
// and `cf.DurableObjectTransaction`. Pure runtime utilities — no IaC deps.
...
export interface SqlCursor<T extends Record<string, SqlStorageValue>> extends Stream.Stream<T> {
	next(): Effect.Effect<{ done?: false; value: T } | { done: true; value?: never }>
...
const fromSqlCursor = <T extends Record<string, SqlStorageValue>>(
	...
	return Object.assign(stream, {
		...
		raw: <U extends SqlStorageValue[]>() => Stream.fromIterableEffect(Effect.sync(() => cursor.raw<U>())),
		get columnNames() {
...
const fromSqlStorage = (sql: cf.SqlStorage): SqlStorage => ({
	exec: <T extends Record<string, SqlStorageValue>>(
		query: string,
		...bindings: any[]
	): Effect.Effect<SqlCursor<T>> => Effect.sync(() => fromSqlCursor(sql.exec<T>(query, ...bindings))),
	get databaseSize() {
...
export interface DurableObjectTransaction {
	...
	get<T = unknown>(keys: string[], options?: cf.DurableObjectGetOptions): Effect.Effect<Map<string, T>>
	list<T = unknown>(options?: cf.DurableObjectListOptions): Effect.Effect<Map<string, T>>
	put<T>(key: string, value: T, options?: cf.DurableObjectPutOptions): Effect.Effect<void>
	put<T>(entries: Record<string, T>, options?: cf.DurableObjectPutOptions): Effect.Effect<void>
	delete(key: string, options?: cf.DurableObjectPutOptions): Effect.Effect<boolean>
	delete(keys: string[], options?: cf.DurableObjectPutOptions): Effect.Effect<number>
	rollback(): Effect.Effect<void>
	getAlarm(options?: cf.DurableObjectGetAlarmOptions): Effect.Effect<number | null>
	setAlarm(scheduledTime: number | Date, options?: cf.DurableObjectSetAlarmOptions): Effect.Effect<void>
...
export interface DurableObjectStorage {
	...
	transaction<T>(closure: (txn: DurableObjectTransaction) => Effect.Effect<T>): Effect.Effect<T>
	getAlarm(options?: cf.DurableObjectGetAlarmOptions): Effect.Effect<number | null>
	setAlarm(scheduledTime: number | Date, options?: cf.DurableObjectSetAlarmOptions): Effect.Effect<void>
	deleteAlarm(options?: cf.DurableObjectSetAlarmOptions): Effect.Effect<void>
...
export const fromDurableObjectTransaction = (txn: cf.DurableObjectTransaction): DurableObjectTransaction => ({
	...
	list: (options?: cf.DurableObjectListOptions) => Effect.tryPromise(() => txn.list(options)),
	put: ((
	...
	getAlarm: (options?: cf.DurableObjectGetAlarmOptions) => Effect.tryPromise(() => txn.getAlarm(options)),
...
export const fromDurableObjectStorage = (storage: cf.DurableObjectStorage): DurableObjectStorage => ({
	get: ((keyOrKeys: string | string[], options?: cf.DurableObjectGetOptions) =>
		Effect.tryPromise(() => storage.get(keyOrKeys as any, options))) as any,
	list: (options?: cf.DurableObjectListOptions) => Effect.tryPromise(() => storage.list(options)),
	put: ((
		keyOrEntries: string | Record<string, unknown>,
		valueOrOptions?: unknown,
		maybeOptions?: cf.DurableObjectPutOptions,
	...
	deleteAll: (options?: cf.DurableObjectPutOptions) => Effect.tryPromise(() => storage.deleteAll(options)),
	transaction: <T>(closure: (txn: DurableObjectTransaction) => Effect.Effect<T>) =>
		Effect.tryPromise(() =>
			storage.transaction((txn) => Effect.runPromise(closure(fromDurableObjectTransaction(txn)))),
		),
