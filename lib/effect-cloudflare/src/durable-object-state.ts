...
// migration:
//   https://github.com/alchemy-run/alchemy-effect/blob/main/packages/alchemy/src/Cloudflare/Workers/DurableObjectState.ts
//
// Context.Service carrying the DO's `state` handle. Provided by the DO bridge
// once per constructor call; `yield* DurableObjectState` from inside any
// method body to access storage, websockets, concurrency control, etc.
...
export class DurableObjectState extends Context.Service<
	DurableObjectState,
	{
		...
		blockConcurrencyWhile<T>(callback: () => Effect.Effect<T>): Effect.Effect<T>
		...
		setWebSocketAutoResponse(maybeReqResp?: cf.WebSocketRequestResponsePair): Effect.Effect<void>
		getWebSocketAutoResponse(): Effect.Effect<cf.WebSocketRequestResponsePair | null>
		getWebSocketAutoResponseTimestamp(ws: cf.WebSocket): Effect.Effect<Date | null>
...
export const fromDurableObjectState = (state: cf.DurableObjectState): DurableObjectState["Service"] => ({
	id: state.id,
	container: state.container,
	storage: fromDurableObjectStorage(state.storage),
	blockConcurrencyWhile: <T>(callback: () => Effect.Effect<T>) =>
		Effect.tryPromise(() => state.blockConcurrencyWhile(() => Effect.runPromise(callback()))),
