...
the site), **deliberately skipped** (the finding tells us *not* to invest), or **strategy**
...
|---|----------------|----------|
| 5 | Adding schema markup had **~zero** impact on AI citations (AIO −4.6%, AI Mode +2.4%, ChatGPT +2.2% — all noise) | We did **not** do a speculative schema buildout. We add only *format-native* structured data where it's the natural fit: `ItemList` for the listicle and the existing `FAQPage` / `BreadcrumbList` components. No Article/Product/etc. on spec. |
...
| 7 | AI Overviews cut clicks to the #1 result by 58% (and accelerating) | The payoff of being *the cited source* keeps rising — this is why GEO is worth doing at all, and why we optimize for the citation, not just the rank. |
