...
export class TemplateMiner {
	...
	constructor(config?: TemplateMinerConfig) {
		...
		this.masker = new LogMasker(maskingInstructions, cfg.maskPrefix, cfg.maskSuffix)
	...
	extractParameters(
		...
		const { regex: templateRegex, paramMap } = this.getTemplateParameterExtractionRegex(
			logTemplate,
			exactMatching,
		)
